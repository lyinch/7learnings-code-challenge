...
The plots were generated with a black background to make them readable in vscode. To change this remove the following line from the notebook:
...
# Challenge
Given historical weather data from multiple stations, the challenge was to predict whether it would snow at each location for a given day.

The solution starts of by querying the data. Then it drops stations and columns which consist predominantly of NaN values. The remaining columns are imputed with the mean of the neighboring data. Additionally, the units got converted to metric such that they are easier to understand and work with.

Before training any model, 10% of the stations were split off and served as a holdout set. The first model was a simple Markov chain per station. At this point it became clear that all the binary columns such as `snow`, or `fog` contain the same values. Their `true` values are spread out over all the months for a given station which makes it very implausible that they track snow. Therefore the Markov chain analysis was not finished properly and the Notebook continues to some pseudocode about testing and feature selection of this model.

The last step was to train another model which considers more features, in this case a simple classification tree. Since time was running out and there's no good predictor about _snowy days_ in the data, it's a mix between code and pseudocode.

With a more in depth data exploration step the issue about the `snow` column could have been catcher before any model training. Additionally, it would be useful to fetch the geographic location of each station such that close-by ones could be pooled together.
